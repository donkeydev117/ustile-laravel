    ...
    <div>
                                ...
                                <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                    <div class="card-header align-items-center  border-bottom-dark px-0">
                                        ...
                                        <div class="icons d-flex">
                                            <button class="btn ml-2 p-0 kt_notes_panel_toggle" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" v-if="$parent.permissions.includes('product-technical-data')">
                                            ...
                                            <div class=" table-responsive" id="printableTable">
                                                <div id="productbrandTable_wrapper" class="dataTables_wrapper no-footer">
                                                    <div class="dataTables_length" id="productbrandTable_length"><label>Show
                                                            <select name="productbrandTable_length" aria-controls="productbrandTable" class="" v-model="limit" v-on:change="fetchData()">
                                                    ...
                                                    <div id="productbrandTable_filter" class="dataTables_filter"><label>Search:<input type="search" class="" placeholder="" aria-controls="productbrandTable" v-model="searchParameter" @keyup="fetchData()"></label></div>
                                                    <table id="productbrandTable" class="display dataTable no-footer" role="grid">
                                                        <thead class="text-body">
                                                            <tr role="row">
                                                                ...
                                                                <th v-if="$parent.permissions.includes('product-technical-data')" class="no-sort sorting_disabled" rowspan="1" colspan="1" aria-label="Action" style="width: 53.1891px;">
                                                                    Action
                                                                </th>
                                                            </tr>
                                                        </thead>
                                                        <tbody class="kt-table-tbody text-dark">
                                                            <tr class="kt-table-row kt-table-row-level-0 odd" role="row" v-for="data in technicalData" v-bind:key="data.id">
                                                                <td class="sorting_1">
                                                                    {{data.id}}
                                                                </td>
                                                                <td>
                                                                ...
                                                                <td>
                                                                    <img :src='data.gallary.detail[0].path' style="width: 50px;" />
                                                                </td>
                                                                <td v-if="$parent.permissions.includes('product-technical-data')">
                                                                    <a href="javascript:void(0)" class=" click-edit" id="click-edit1" data-toggle="tooltip" title="" data-placement="right" data-original-title="Check out more demos" @click="editData(data)"><i class="fa fa-edit"></i></a>
                                                                    <a class="" href="#" @click="deleteData(data.id)"><i class="fa fa-trash"></i></a>
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
        ...
        <div class="offcanvas offcanvas-right kt-color-panel p-5 kt_notes_panel" v-if="display_form" :class="display_form ? 'offcanvas-on' : ''">
            <div class="offcanvas-header d-flex align-items-center justify-content-between pb-3">
                <h4 class="font-size-h4 font-weight-bold m-0">Add Technical Data</h4>
            ...
            <form id="myform">
                <div class="row">
                    <div class="col-12">
                        <div class="form-group ">
                            <label class="text-dark">Technial Data Name</label>
                            <input type="text"  v-model="tData.name" class="form-control" />
                            <small class="form-text text-danger" v-if="errors.has('name')" v-text="errors.get('name')"></small>
                        </div>

                        <div class="form-group">
                            <button type="button" class="btn btn-primary" @click="toggleImageSelect()">Upload Technical Data Media</button>
                ...
                <button type="button" @click="addUpdateData()" class="btn btn-primary">Submit</button>
            </form>
        </div>
...
export default {
    ...
    methods: {
        fetchData(page_url) {
            ...
            let vm = this;
            page_url = page_url || "/api/admin/technical";
        ...
        deleteData(id) {
            if (confirm('Are You Sure?')) {
                this.$parent.loading = true;
                axios.delete(`/api/admin/technical/${id}`, this.token)
                    .then(res => {
        ...
        addUpdateData() {
            this.$parent.loading = true;
            var url = '/api/admin/technical';
            if (this.edit === false) {
                // Add
                this.request_method = 'post'
            } else {
                // Update
                var url = '/api/admin/technical/' + this.tData.id;
                this.request_method = 'put'
            }

            axios[this.request_method](url, {
                    name: this.tData.name,
                    media: this.tData.image,
                }, this.token)
                .then(res => {
                    if (res.data.status == "success") {
                        this.display_form = 0;
        ...
        editData(data) {
            ...
            this.display_form = 1;
            this.errors = new ErrorHandling();
            this.tData.id = data.id;
            this.tData.name = data.name;
            this.tData.image = data.image;
            this.gallary_path = data.gallary.detail[0].path;
        },
        clearForm() {
            this.edit = false;
            this.tData.id = null;
        ...
        setImage(gallary){
            ...
            this.tData.image = gallary.gallary_id;
        }
    },
    mounted() {
